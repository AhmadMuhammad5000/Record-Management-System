<?php  require_once "../database/db.php";
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update"])){
        ...
        if(empty($_POST['name']) || empty($_POST['result'])){
            ...
            $sql = $conn->prepare("UPDATE lab_records set username =?,result =? address =? phone =? gender =? WHERE reference_id=?");
            $sql->bind_param("sssssi",$name,$result,$address,$phone,$gender,$ref);
            $sql->execute();
            ...
            if($sql){
                ...
                header("location: edit_lab_record?ref=".$ref.".php");
            }
